...
class GlobalAttention(nn.Module):
    ...
    def forward(self, a, b, c):
        ...
        self.rel_h = nn.Parameter(torch.randn([1, self.chanel_in, 1, height]), requires_grad=True).cuda()
        self.rel_w = nn.Parameter(torch.randn([1, self.chanel_in, width, 1]), requires_grad=True).cuda()
        ...
        content_position = (self.rel_h + self.rel_w).view(1, self.chanel_in, -1)  # B,C,N
        content_position = torch.matmul(content_position.permute(0,2,1),proj_query.permute(0,2,1)) # B ,N ,N
        feature_similarity = torch.bmm(proj_query, proj_key)  # B, N, N
        energy = feature_similarity + content_position
        mask = c.view(m_batchsize, -1, width * height)  # B, C, N

        mask = mask.repeat(1, height * width, 1).permute(0, 2, 1)  # B, 1, H, W -> B, C, H, W // B
        feature_pruning = energy * mask
...
class GlobalAttentionPatch(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.gamma = torch.tensor([1.0], requires_grad=True).cuda()
        self.channel1 = nn.Conv2d(in_channels=in_dim, out_channels=in_dim//8, kernel_size=1,stride=1,padding=0)
        self.channel2 = nn.Conv2d(in_channels=in_dim//8, out_channels=in_dim, kernel_size=1, stride=1,padding=0)
    ...
    def forward(self, x, y, m):
        ...
        ca = self.channel1(x) 
        ca = self.channel2(ca) 
        channel_correlation = self.sigmoid(ca).view(feature_size[0], -1, feature_size[2] * feature_size[3])
