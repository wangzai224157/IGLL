...
class svd_loss(nn.Module):
    def __init__(self):
        ...
        self.uloss , self.sloss , self.vloss , self.sloss_sum= None, None, None, None

    def forward(self, gen, tar):
        u1,s1,v1 = torch.svd(gen)
        u2,s2,v2 = torch.svd(tar)
        self.sloss = torch.mean((s2-s1)**2)
        self.sloss_sum = torch.sum(s2) - torch.sum(s1)
        self.uloss =  torch.norm(u1-u2,p=2)
        self.vloss =  torch.norm(v1-v2 ,p=2)
        
        #return  self.uloss + self.sloss + self.vloss + self.sloss_sum
        return   self.sloss
